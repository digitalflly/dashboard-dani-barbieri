...
export default function FunisPage({ dash, funis }: { dash: Dashboard; funis: FunisVM }) {
  ...
  const v = funis
  const inv = v.invest
  const isSeg = inv.isSeg

  // posicionamento da grade de investimento (funil Seguidores reorganiza as células)
  const investGrid: CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16, ...(isSeg ? { alignItems: 'stretch' } : {}) }
  const funnelCell: CSSProperties = { gridColumn: 1, gridRow: isSeg ? '1 / span 2' : 1, minWidth: 0, display: 'flex' }
  const adsetCell: CSSProperties = { gridColumn: 2, gridRow: 1, minWidth: 0, display: 'flex' }
  ...
  const adsetWrap: CSSProperties = isSeg
    ? { overflow: 'auto', margin: '0 -4px', height: 430 }
    : { overflow: 'auto', margin: '0 -4px', flex: '1 1 0', minHeight: 0 }
  const adsWrap: CSSProperties = isSeg ? { overflow: 'auto', margin: '0 -4px', maxHeight: 625 } : { overflowX: 'auto', margin: '0 -4px' }
  const adsTable: CSSProperties = isSeg ? { borderCollapse: 'collapse', width: '100%', tableLayout: 'fixed' } : { borderCollapse: 'collapse', width: '100%', minWidth: 660 }
  const adsNameCol: CSSProperties = { ...thPlain, textAlign: 'left', padding: '0 12px 10px 4px', ...(isSeg ? { width: '38%' } : {}) }
          ...
          {inv.investShow && (
            <>
              ...
              <div style={investGrid}>
                ...
                <div style={funnelCell}>
                  <div className="b-card" style={{ padding: '22px 24px', flex: 1, width: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={eyebrow}>{inv.investEyebrow}</div>

                    {inv.convShow && (
                      <>
                        <div style={{ ...cardTitle, marginBottom: 16 }}>Dados de conversão</div>
                    ...
                    {inv.turbinaShow && (
                      <>
                        <div style={{ ...cardTitle, marginBottom: 16 }}>Turbinamento</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>
                          {inv.turbinaSummary.map((t, i) => (
                            <div key={i} style={{ border: '1px solid var(--border-soft)', borderRadius: 5, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                              <div style={{ fontSize: 9.5, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--text-muted)', lineHeight: 1.2 }}>{t.label}</div>
                              <div data-keepcase="1" style={{ fontSize: 17, fontWeight: 600, color: 'var(--text-strong)', letterSpacing: '-0.02em' }}>{t.value}</div>
                            </div>
                          ))}
                        </div>
                ...
                {/* adsets table */}
                <div style={adsetCell}>
                  <div className="b-card" style={{ padding: '22px 24px', minWidth: 0, flex: 1, width: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={eyebrow}>{inv.investEyebrow}</div>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                      <div style={cardTitle}>Conjuntos de anúncio</div>
                      ...
                      <table style={{ borderCollapse: 'collapse', width: '100%', tableLayout: 'fixed' }}>
                        <thead>
                          <tr style={{ textAlign: 'right' }}>
                            ...
                            <th style={th({ textAlign: 'right', padding: '0 4px 10px 8px' })}>{inv.adsetResultLabel}</th>
                          </tr>
                        </thead>
                        <tbody>
                ...
                {/* ads table */}
                <div style={adsCell}>
                  <div className="b-card" style={{ padding: '22px 24px', minWidth: 0, flex: 1, width: '100%' }}>
                    <div style={eyebrow}>{inv.investEyebrow}</div>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                      <div style={cardTitle}>Anúncios</div>
                      <span data-keepcase="1" style={{ fontSize: 11, color: 'var(--text-muted)' }}>{inv.adsStatus}</span>
                    </div>
                    <div style={adsWrap}>
                      <table style={adsTable}>
                        <thead>
                          <tr style={{ textAlign: 'right' }}>
